...
Sunbreak V3 is live at **https://kpop.8westventures.com** on Coastline. Its verified implementation release, **[`884e24d`](https://github.com/Seckcey/kannon-fps/commit/884e24d422e1666a5caccc90a642da7b58de594d)**, was merged through [PR #4](https://github.com/Seckcey/kannon-fps/pull/4) with [successful main CI](https://github.com/Seckcey/kannon-fps/actions/runs/34209569042). [/health](https://kpop.8westventures.com/health) reports the canonical current deployed build, including later documentation closeouts. Cloudflare Tunnel uses **HTTP `127.0.0.1:14350`**. V3 passed 79 automated tests, asset validation, TypeScript and production build checks, separate controller/real-input checks, and deployed HTTP/WebSocket/public HTTPS/WSS verification with matching model hashes. Real-device performance/controls acceptance and a two-household playtest remain. The previous V2 release introduced the visual/practice foundation through PR #3; see the [release record](docs/RELEASE.md) for the separate evidence and limits of each release.

## Sunbreak V3 art and motion

The next [combat-feedback candidate](docs/COMBAT_FEEDBACK.md) adds per-pellet impacts, shield/armor reactions and correctly placed respawn effects. Its local checks are separate from the deployed V3 release above; Linux and deployment acceptance are still pending.
