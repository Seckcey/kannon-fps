...
The bots and public-games update adds a prominent **Play against bots** entry, optional AI filling in casual friends matches, and **Available Games** for joining public lobbies without an invite. Public/private selection is explicit; ranked crew matches remain private and human-only.
...
1. **Automated correctness:** 179 client/server/transport tests, asset validation, TypeScript and production build pass. Public-room tests cover directory privacy, admission and capacity, bot replacement, readiness, recovery, unranked persistence, expiry and separate polling rate limits. Map tests cover every spawn to both upstairs floors, all stair/access routes, support/occlusion and complete bounded practice rounds at every difficulty.
...
Solo practice retains three rivals and Relaxed, Balanced and Challenging difficulty. Rivals use ordinary combat rules and can win or draw. They navigate both houses and all supported stairs without general climbing or jumping. Practice creates no bot profiles, ranked results or saved match history. Casual friends matches can fill to four participants with bots. Public lobbies appear in Available Games until the host starts; private rooms and ranked crews remain invitation-controlled.
