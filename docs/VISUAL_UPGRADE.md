...
The first complete Sunbreak V2 slice shipped through PR #3 with local/CI, desktop/touch, public browser and isolated Linux acceptance recorded in [RELEASE.md](RELEASE.md). [/health](https://kpop.8westventures.com/health) is the canonical current deployed build identity. Those historical results do not establish that a later candidate has shipped.

The current [art-depth candidate](ART_DEPTH.md) is pending release. It includes asymmetric exterior terraces and a connected coastal backdrop, distinct facade/cover construction, fuller tree crowns, a more athletic weapon-ready posture and less mechanical armor silhouettes. The arena collision and game rules remain unchanged. Its own source, controller, rendered-input and performance evidence is recorded separately from V2's release evidence.
