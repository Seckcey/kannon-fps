...
- Public acceptance on the deployed site ([town-v2-live.json](verification/town-v2-live.json)) reaches a prepared bot match with both v2 models downloaded through HTTPS and no application errors. The first deployment exposed a policy conflict with the texture transcoder, fixed the same day by serving its worker from this origin with its own narrow policy; see [RELEASE.md](RELEASE.md).
