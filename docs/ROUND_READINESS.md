...
[Isolated Linux acceptance](verification/round-readiness-linux.json) proves the eight-player readiness handshake, sustained combat and recovery, plus all three practice difficulties. [Public rendered acceptance](verification/round-readiness-live.json) proves the deployed ready screen, native capture through the full countdown, moving/firing rivals, exact model downloads and explicit exit. These checks use separate environments and do not establish physical-phone performance or a two-household family match. Exact implementation, deployment preservation and evidence limits are recorded in [Release verification](RELEASE.md).
