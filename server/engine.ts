...
import { RULES, type GameEvent, type InputFrame, type Phase, type PlayerState, type Profile, type Vec3, type WorldSnapshot } from '../shared/protocol.js';
...
export class MatchEngine {
  ...
  addPlayer(profile: Profile, now = Date.now(), bot = false): PlayerState {
    ...
    if (existing) {
      existing.connected = true; existing.name = profile.name; existing.lastInputSeq = 0;
    ...
    const state: PlayerState = { ...profile, x: 0, y: 0, z: 0, yaw: 0, pitch: 0, vx: 0, vy: 0, vz: 0, health: RULES.health, shield: RULES.shield, slot: 1, ammoAR: RULES.arMagazine, ammoShotgun: RULES.shotgunMagazine, heals: RULES.healCharges, kills: 0, deaths: 0, connected: true, respawnAt: 0, protectedUntil: 0, healingUntil: 0, reloadingUntil: 0, lastInputSeq: 0, ...(bot ? { bot: true } : {}) };
  ...
  finish(reason: string, now = Date.now(), abandoned = false): void {
    ...
    const eligible = [...this.players.values()].filter((p) => !p.bot);
  ...
  private respawn(player: PlayerState, now: number, announce = true) {
    ...
    Object.assign(player, { x: spawn.x, y: spawn.y, z: spawn.z, yaw: spawn.yaw, pitch: 0, vx: 0, vy: 0, vz: 0, health: RULES.health, shield: RULES.shield, slot: 1, ammoAR: RULES.arMagazine, ammoShotgun: RULES.shotgunMagazine, heals: RULES.healCharges, respawnAt: 0, protectedUntil: now + RULES.protectionMs, healingUntil: 0, reloadingUntil: 0 });
    if (player.bot) {
      const index = [...this.players.values()].filter((p) => p.bot).findIndex((p) => p.id === player.id);
      const positions = [{ x: -10, z: 8 }, { x: 0, z: 0 }, { x: 10, z: 8 }];
      Object.assign(player, positions[index % positions.length], { yaw: Math.PI, protectedUntil: 0 });
    }
