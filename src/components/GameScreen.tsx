...
export default function GameScreen({ connection, settings, setSettings, player, onLeave }: { connection: ArenaConnection; settings: Settings; setSettings: (value: Settings) => void; player: PlayerSession; onLeave: () => void }) {
  ...
  useEffect(() => {
    ...
    const synchronize = () => {
      if (!active) return;
      const current = connection.getSnapshot();
      controller.setGameplayBlocked(current.room?.phase !== 'playing');
    ...
    return () => { active = false; unsubscribe(); ready.dispose(); preparation.current = null; syncControls.current = () => {}; clearInterval(send); clearInterval(time); window.removeEventListener('keydown', down); window.removeEventListener('keyup', up); view?.dispose(); controller.dispose(); game.current = null; };
  }, [connection]);
  ...
  return <main className={`game-screen ${touch ? 'touch-game' : ''} ${damaged ? 'taking-damage' : ''}`}>
    ...
    {(loading || loadError) && <div className="game-cover"><h2>{loadError ? 'Graphics unavailable' : 'Entering Sunbreak…'}</h2><p>{loadError || 'Preparing the arena and your loadout.'}</p><button className={`button ${loadError ? 'primary' : 'secondary'}`} onClick={onLeave}>Back to menu</button></div>}
    <div className="game-hud">
      ...
      {!dead && !finished && !preparing && !countdown && <div className={`crosshair ${hit ? 'hit' : ''} ${me?.slot === 2 ? 'shotgun-crosshair' : ''}`}><i/><i/><i/><i/>{hit && <span>×</span>}</div>}
    ...
    {touch && input && !paused && !finished && !showSettings && !preparing && !countdown && !loading && !loadError && state.status === 'connected' && <TouchControls input={input} healingSlot={me?.slot === 3}/>}
