...
export default function GameScreen({ connection, settings, setSettings, player, onLeave }: { connection: ArenaConnection; settings: Settings; setSettings: (value: Settings) => void; player: PlayerSession; onLeave: () => void }) {
  ...
  const phase = snapshot?.phase || state.room?.phase;
  ...
  useEffect(() => {
    ...
    try {
      view = new GameView(container.current, { input: controller, getSnapshot: () => connection.getSnapshot().snapshot, getPlayerId: () => connection.getSnapshot().playerId, settings, onStats: setStats, onAssetsReady: () => setLoading(false), onError: message => { setLoadError(message); setLoading(false); } });
      game.current = view; setInput(controller);
    } catch { setLoadError('This device could not start 3D graphics. Try an up-to-date browser with hardware acceleration enabled.'); controller.dispose(); setLoading(false); }
  ...
  const resume = () => { setPaused(false); input?.setPaused(false); if (!touch) input?.requestPointerLock(); };
  ...
  return <main className={`game-screen ${touch ? 'touch-game' : ''} ${damaged ? 'taking-damage' : ''}`}>
    ...
    <div className="game-hud">
      ...
      {!dead && !countdown && !finished && (me?.protectedUntil || 0) > serverNow && <div className="protection-label"><Icon name="shield" size={14}/> Spawn protection</div>}
      {!touch && !paused && !showSettings && !finished && !countdown && !stats.locked && !loadError && <button className="mouse-prompt" onClick={resume}><Icon name="play" size={18}/> Click to play <span>WASD move · Mouse aim · 1 / 2 / 3 loadout · Esc releases mouse</span></button>}
      {!touch && <span className="game-bottom-hint">Hold Tab for scores · R reload · Shift sprint · Space jump</span>}
    </div>
    {touch && input && !paused && !finished && !showSettings && !countdown && <TouchControls input={input} healingSlot={me?.slot === 3}/>}
    {touch && <div className="portrait-hint"><Icon name="phone" size={30}/><span>Turn your phone sideways for the best view.</span></div>}
    {countdown && <div className="center-message"><span>Get ready</span><strong>{Math.max(1, Math.ceil(snapshot?.timeRemaining ?? 3))}</strong><p>{state.room?.practice ? 'Warm up. Find your rhythm.' : 'Same loadout. Let’s settle it.'}</p></div>}
    {dead && !finished && <div className="respawn-message"><h2>Back in {Math.max(1, Math.ceil(((me?.respawnAt || serverNow) - serverNow) / 1000))}</h2><p>Fresh loadout. Next life.</p></div>}
