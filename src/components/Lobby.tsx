...
export function Lobby({ room, playerId, onStart, onLeave, busy }: { room: RoomSnapshot; playerId: string; onStart: () => void; onLeave: () => void; busy: boolean }) {
  ...
  return <section className="room-lobby"><div className="lobby-copy"><button className="text-button back-button" onClick={onLeave}><Icon name="leave" size={18}/> Leave room</button><h1>{room.practice ? 'Find your rhythm.' : 'Bring your rivals.'}</h1><p>{room.practice ? 'Meet Sunbreak. Warm up your aim on three training drones.' : 'One invitation. Your whole crew. The next round is yours.'}</p>
    ...
    <div className="match-summary"><div><span>Arena</span><strong>Sunbreak Courtyard</strong></div><div><span>Mode</span><strong>{room.practice ? 'Solo practice' : room.ranked ? 'Ranked crew match' : 'Casual free-for-all'}</strong></div><div><span>Rules</span><strong>5 minutes · First to 15</strong></div></div>
    <Loadout compact/>
  </div><div className="roster-panel"><div className="roster-heading"><h2>{room.practice ? 'The practice range' : 'In the room'}</h2><span>{people.length} / {room.practice ? 1 : 8}</span></div><ul className="roster-list">{room.players.map(p => <li key={p.id}><div className="player-avatar" style={{ '--player-color': p.color } as React.CSSProperties}><Icon name={p.bot ? 'target' : 'shield'} size={24}/></div><div><strong>{p.name}</strong><span>{p.bot ? 'Training target' : p.id === room.hostId ? 'Room host' : 'Ready to play'}{p.id === playerId ? ' · You' : ''}</span></div><span className={`online-dot ${p.connected ? '' : 'offline'}`} title={p.connected ? 'Connected' : 'Reconnecting'}/></li>)}</ul>
