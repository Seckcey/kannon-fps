...
export function Lobby({ room, playerId, onStart, onLeave, busy }: { room: RoomSnapshot; playerId: string; onStart: () => void; onLeave: () => void; busy: boolean }) {
  ...
  return <section className="room-lobby"><div className="lobby-copy"><button className="text-button back-button" onClick={onLeave}><Icon name="leave" size={18}/> Leave room</button><h1>{room.practice ? 'Find your rhythm.' : 'Bring your rivals.'}</h1><p>{room.practice ? 'Meet Kannon Town. Three AI rivals. A real round to sharpen your game.' : 'One invitation. Your whole crew. The next round is yours.'}</p>
    ...
    <div className="match-summary"><div><span>Arena</span><strong>Kannon Town</strong></div><div><span>Mode</span><strong>{room.practice ? `${practiceLevelName(room.practiceDifficulty)} practice` : room.ranked ? 'Ranked crew match' : 'Casual free-for-all'}</strong></div><div><span>Rules</span><strong>5 minutes · First to 15</strong></div></div>
    <Loadout compact/>
