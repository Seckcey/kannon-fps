import { WORLD_VERSION, type ClientMessage, type GameEvent, type RoomSnapshot, type ServerMessage, type WorldSnapshot } from '../../shared/protocol';
...
export class ArenaConnection {
  ...
  connect = (): Promise<void> => {
    ...
    socket.onopen = () => { if (socket === this.socket && !this.closed) socket.send(JSON.stringify({ type: 'hello', token: this.token, readyProtocol: 1, worldVersion: WORLD_VERSION } satisfies ClientMessage)); };
